...
import { CalendarDays, Camera, CheckCircle2, ChevronRight, FileText, Heart, MapPin, Settings, Star, Users } from 'lucide-vue-next'
...
const customInterestTags = computed(() => parseInterestTags().filter(t => !suggestedInterests.includes(t)))
...
function addCustomInterest() {
  ...
  if (!hasInterest(tag)) {
    const tags = parseInterestTags()
    tags.push(tag)
    profile.interests = tags.join('、')
  }
  customInterestInput.value = ''
}
function removeCustomInterest(tag: string) {
  const tags = parseInterestTags().filter(t => t !== tag)
  profile.interests = tags.join('、')
}
