...
<template><div class="check-page"><div class="container"><div class="check-head"><span class="eyebrow">ON-SITE CHECK IN</span><h1>扫码签到</h1><p>二维码由活动发起人在个人页「我的活动」中生成，现场展示给已报名用户扫描。</p></div><div class="check-card"><div :class="['scanner',{success:scanned}]"><div class="scan-frame"><QrCode v-if="!scanned" :size="100" /><CheckCircle2 v-else :size="90" /><i></i><span></span></div><div class="scan-line" v-if="!scanned"></div></div><div class="check-info"><template v-if="!scanned"><span class="status-dot">等待扫描</span><h2>输入或粘贴签到码</h2><p><MapPin :size="16" />扫码会自动带入 code；无法扫码时可手动输入</p><input v-model.trim="code" class="input" placeholder="签到二维码中的 code" /><p v-if="error" class="auth-error">{{ error }}</p><button class="btn btn-primary" @click="scan"><Camera :size="18" />确认签到</button><div class="privacy"><ShieldCheck :size="18" /><span>签到码和活动绑定，未报名用户或过期二维码会被拒绝。</span></div></template><template v-else><span class="status-dot done">签到成功</span><h2>嗨，{{ user?.nickname || '伙伴' }}，现场见！</h2><p>{{ message }}</p><RouterLink to="/profile" class="btn btn-dark">返回个人页</RouterLink></template></div></div></div></div></template>
<style scoped>
