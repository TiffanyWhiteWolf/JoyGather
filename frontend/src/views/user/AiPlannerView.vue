...
const error = ref('')
const plan = ref({ title: '', introduction: '', tags: [] as string[], schedule: [] as string[], safetyNote: '' })
      ...
      <div class="planner-grid">
        ...
        <section :class="['result-panel', { empty: !generated }]">
          ...
          <div v-else class="generated">
            ...
            <p>{{ plan.safetyNote }}</p>
            <button class="btn btn-primary publish" @click="router.push('/create?ai=1')">采用方案并继续编辑</button>
          </div>
        </section>
      </div>
    </div>
