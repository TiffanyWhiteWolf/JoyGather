...
<template>
  <div class="detail-page"><div class="detail-hero"><div class="container"><RouterLink to="/discover" class="back"><ChevronLeft :size="17" />返回活动列表</RouterLink><div class="cover-wrap"><img :src="activity.cover" :alt="activity.title" /><div class="cover-gradient"></div><div class="cover-title"><div class="tag-row"><span>{{ activity.category }}</span><span v-for="tag in activity.tags" :key="tag"># {{ tag }}</span></div><h1>{{ activity.title }}</h1><p>{{ activity.summary }}</p></div><div class="cover-actions"><button @click="liked=!liked"><Heart :fill="liked?'currentColor':'none'" :size="18" /></button><button @click="shareActivity"><Share2 :size="18" /></button></div></div></div></div>
    ...
    <div class="container detail-layout"><div class="detail-main"><section class="detail-section"><h2>关于这场活动</h2><p>{{ activity.description }}</p><div class="facts"><div><CalendarDays /><span><b>{{ activity.date }}</b><small>活动日期</small></span></div><div><Clock /><span><b>{{ activity.time }}</b><small>活动时间</small></span></div><div><MapPin /><span><b>{{ activity.location }}</b><small>{{ activity.district }} · 距你 {{ activity.distance }}km</small></span></div><div><Users /><span><b>{{ activity.joined }} 人已报名</b><small>上限 {{ activity.capacity }} 人</small></span></div></div></section><section class="detail-section organizer"><img :src="activity.organizer.avatar" /><div><small>活动发起人</small><h3>{{ activity.organizer.nickname }} <Check :size="14" /></h3><p>{{ activity.organizer.bio }} · 信用分 {{ activity.organizer.credit }}</p></div><button class="btn btn-outline" @click="contactOrganizer"><MessageCircle :size="16" />联系 Ta</button></section><RouterLink :to="`/create?clone=${activity.id}`" class="clone-link"><Copy :size="16" />克隆这场活动并修改后再次发布</RouterLink></div>
