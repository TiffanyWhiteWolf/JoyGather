...
const route = useRoute(); const router = useRouter(); const app = useAppStore(); const confirmOpen = ref(false); const cancelOpen = ref(false); const liked = ref(false); const safetyAccepted = ref(false); const unfollowConfirmOpen = ref(false)
const formError = ref('')
...
async function contactOrganizer() {
  await router.push({ path: '/messages', query: { userId: activity.value.organizer.id } })
}

async function toggleFollowOrganizer() {
  const organizerId = activity.value.organizer.id
  if (app.followedIds.includes(organizerId)) {
    unfollowConfirmOpen.value = true
    return
  }
  try {
    await apiPost(`/follows/${organizerId}`, {})
    app.addFollowedId(organizerId)
...
async function confirmUnfollowOrganizer() {
  const organizerId = activity.value.organizer.id
  try {
    await apiDelete(`/follows/${organizerId}`)
    app.removeFollowedId(organizerId)
...
<template>
  <div class="detail-page"><div class="detail-hero"><div class="container"><RouterLink to="/discover" class="back"><ChevronLeft :size="17" />返回活动列表</RouterLink><div class="cover-wrap"><img :src="activity.cover" :alt="activity.title" /><div class="cover-gradient"></div><div class="cover-title"><div class="tag-row"><span>{{ activity.category }}</span><span v-for="tag in activity.tags" :key="tag"># {{ tag }}</span></div><h1>{{ activity.title }}</h1><p>{{ activity.summary }}</p></div><div class="cover-actions"><button @click="liked=!liked"><Heart :fill="liked?'currentColor':'none'" :size="18" /></button><button @click="shareActivity"><Share2 :size="18" /></button></div></div></div></div>
    ...
    <div v-if="unfollowConfirmOpen" class="modal-mask" @click.self="unfollowConfirmOpen=false">
      <div class="confirm-modal">
        <span class="modal-icon cancel"><X /></span>
        <h2>确定取消关注 {{ activity.organizer.nickname }}？</h2>
        <p>取消后对方将从你的关注列表中移除，双方好友关系也会解除。</p>
        <div>
          <button class="btn btn-outline" @click="unfollowConfirmOpen=false">保留关注</button>
          <button class="btn btn-dark" @click="confirmUnfollowOrganizer">确认取消</button>
        </div>
      </div>
    </div>
  </div>
