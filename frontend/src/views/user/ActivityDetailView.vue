...
import { BookOpen, CalendarDays, Check, ChevronLeft, Clock, Copy, Heart, MapPin, MessageCircle, Share2, ShieldCheck, Star, UserCheck, Users, X } from 'lucide-vue-next'
import { computed, onMounted, reactive, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { formatPrice, percent } from '@/lib/utils'
import { useAppStore } from '@/stores/app'
import { apiDelete, apiGet, apiPost } from '@/lib/api'
import type { Activity, ActivityAfterEvent, User } from '@/types'
...
async function loadActivity() {
  const [act, user, feedback] = await Promise.all([
    apiGet<Activity>(`/activities/${route.params.id}`),
    apiGet<User>('/auth/me').catch(() => null),
    apiGet<ActivityAfterEvent>(`/activities/${route.params.id}/after-event`).catch(() => null),
  ])
...
<template>
  <div class="detail-page"><div class="detail-hero"><div class="container"><RouterLink to="/discover" class="back"><ChevronLeft :size="17" />返回活动列表</RouterLink><div class="cover-wrap"><img :src="activity.cover" :alt="activity.title" /><div class="cover-gradient"></div><div class="cover-title"><div class="tag-row"><span>{{ activity.category }}</span><span v-for="tag in activity.tags" :key="tag"># {{ tag }}</span></div><h1>{{ activity.title }}</h1><p>{{ activity.summary }}</p></div><div class="cover-actions"><button @click="liked=!liked"><Heart :fill="liked?'currentColor':'none'" :size="18" /></button><button @click="shareActivity"><Share2 :size="18" /></button></div></div></div></div>
    <div v-if="showRejectionBanner" class="container"><div class="rejection-banner"><div class="rejection-icon"><X :size="22" /></div><div class="rejection-body"><b>{{ activity.status === '已下架' ? '你的活动未通过审核' : '管理员建议修改活动' }}</b><p>{{ activity.offlineReason || '管理员未填写具体原因' }}</p></div><div class="rejection-actions"><RouterLink :to="`/create?clone=${activity.id}`" class="btn btn-primary btn-sm">根据反馈修改并重新提交</RouterLink></div></div></div>
    <div class="container detail-layout"><div class="detail-main"><section class="detail-section"><h2>关于这场活动</h2><p>{{ activity.description }}</p><div class="facts"><div><CalendarDays /><span><b>{{ activity.date }}</b><small>活动日期</small></span></div><div><Clock /><span><b>{{ activity.time }}</b><small>活动时间</small></span></div><div><MapPin /><span><b>{{ activity.location }}</b><small>{{ activity.district }} · 距你 {{ activity.distance }}km</small></span></div><div><Users /><span><b>{{ activity.joined }} 人已报名</b><small>上限 {{ activity.capacity }} 人</small></span></div></div></section><section v-if="activity.status==='已结束' || afterEvent?.summary" class="detail-section after-teaser"><div class="after-icon"><BookOpen /></div><div><small>ACTIVITY MOMENTS</small><h2>{{ afterEvent?.summary?.title || '活动结束了，故事才刚刚开始' }}</h2><p v-if="afterEvent?.summary">{{ afterEvent.summary.content.slice(0,90) }}{{ afterEvent.summary.content.length>90?'…':'' }}</p><p v-else>查看发起人的图文总结，或在评价期内分享你的真实体验。</p><span v-if="afterEvent?.reviewCount"><Star fill="currentColor" />{{ afterEvent.averageRating }} · {{ afterEvent.reviewCount }} 条评价</span></div><RouterLink :to="`/activities/${activity.id}/after-event`" class="btn btn-primary">{{ afterEvent?.canPublishSummary && !afterEvent.summary ? '发布活动总结' : '查看回顾与评价' }}</RouterLink></section><section class="detail-section organizer"><img :src="activity.organizer.avatar" /><div><small>活动发起人</small><h3>{{ activity.organizer.nickname }} <Check :size="14" /></h3><p>{{ activity.organizer.bio }} · 信用分 {{ activity.organizer.credit }}</p></div><button class="btn btn-outline" @click="contactOrganizer"><MessageCircle :size="16" />联系 Ta</button></section><RouterLink :to="`/create?clone=${activity.id}`" class="clone-link"><Copy :size="16" />克隆这场活动并修改后再次发布</RouterLink></div>
...
.after-teaser{display:flex;align-items:center;gap:16px;background:linear-gradient(135deg,#fff8f4,#f5f1ff)}.after-icon{width:52px;height:52px;flex:0 0 52px;border-radius:15px;background:var(--color-primary);color:#fff;display:grid;place-items:center}.after-teaser>div:nth-child(2){flex:1}.after-teaser small{color:var(--color-primary);font-size:9px;font-weight:900;letter-spacing:.1em}.after-teaser h2{margin:5px 0 7px}.after-teaser p{margin:0;color:var(--color-ink-soft);font-size:11px;line-height:1.6}.after-teaser span{margin-top:8px;color:#d99b0b;display:flex;align-items:center;gap:5px;font-size:10px}.after-teaser span svg{width:14px}
