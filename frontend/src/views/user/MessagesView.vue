...
import { ChevronDown, ChevronUp, File, Image, MapPin, MessageCircle, MoreHorizontal, Search, Send, ShieldBan, ShieldCheck, Smile, UserCheck, UserPlus, UserRoundCheck, UserRoundMinus, UserRoundPlus, UserX, X } from 'lucide-vue-next'
...
const canSend = computed(() => {
  ...
  if (!fid) return true
  return app.friendIds.includes(fid)
})
...
function openProfile() {
  void router.push(active.value.type === '小队' ? '/teams' : '/messages')
}
      ...
      <main class="chat-main">
        ...
        <div ref="messageList" class="messages">
          ...
          <div v-for="message in active.messages" :key="message.id" :class="['message', { mine: message.mine }]">
            <img v-if="!message.mine" :src="message.senderAvatar || active.avatar" alt="发送者头像" />
        ...
        <div v-else class="composer">
          ...
          <div class="compose-input">
            <textarea ref="messageInput" v-model="text" :disabled="!active.id || sending || !canSend" :placeholder="canSend ? '输入消息，Enter 发送，Shift + Enter 换行' : '你们还不是好友，无法发送消息'" @keydown.enter="handleEnter"></textarea>
