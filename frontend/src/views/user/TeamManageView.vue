...
import { AlertTriangle, ArrowLeft, Check, Crown, Edit3, Flag, Image, Shield, Trash2, Users, X } from 'lucide-vue-next'
...
const editForm = reactive({ name: '', description: '', tags: '', capacity: 80, joinMode: '公开加入' as Team['joinMode'], cover: '' })
const editError = ref('')
...
async function dissolveTeam() {
  ...
  try {
    ...
    dissolveError.value = e instanceof Error ? e.message : '解散失败'
  } finally {
...
<template>
  ...
  <div v-else class="container team-manage">
    ...
    <section v-if="tab === 'info'" class="manage-panel">
      <template v-if="!editMode">
        ...
        <div v-if="isOwner" class="info-actions">
          ...
          <button class="btn btn-sm dissolve-btn" @click="openDissolve"><Trash2 :size="14" />解散小队</button>
        </div>
      </template>
      <template v-else>
        <h3>编辑小队信息</h3>
        <div class="edit-form">
          <div class="input-group"><label>名称</label><input v-model="editForm.name" class="input" /></div>
          <div class="input-group"><label>简介</label><textarea v-model="editForm.description" class="textarea" /></div>
          <div class="input-group"><label>标签（顿号分隔）</label><input v-model="editForm.tags" class="input" /></div>
          <div class="form-grid">
            <div class="input-group"><label>人数上限</label><input v-model.number="editForm.capacity" class="input" type="number" /></div>
            <div class="input-group"><label>加入方式</label><select v-model="editForm.joinMode" class="select"><option>公开加入</option><option>审核加入</option></select></div>
          </div>
          <div class="input-group"><label>封面 URL</label><input v-model="editForm.cover" class="input" /></div>
    ...
    <div v-if="dissolveConfirm" class="reject-modal dissolve-modal" @click.self="dissolveConfirm = false">
      <div>
        <h3>解散小队</h3>
        <p>解散后所有成员将被移除，群聊和队内活动将停止使用。请输入“解散小队”确认。</p>
        <input v-model.trim="dissolveText" class="input" autocomplete="off" />
        <p v-if="dissolveError" class="form-error">{{ dissolveError }}</p>
        <footer>
          <button class="btn btn-outline btn-sm" :disabled="dissolveBusy" @click="dissolveConfirm = false">取消</button>
          <button class="btn btn-sm danger-confirm" :disabled="dissolveBusy || dissolveText.trim() !== '解散小队'" @click="dissolveTeam">{{ dissolveBusy ? '解散中...' : '确认解散' }}</button>
        </footer>
      </div>
    </div>
...
.dissolve-btn{background:#ffeaed;color:var(--color-danger)}
...
.form-error{color:var(--color-danger);font-size:12px;margin:8px 0 0}.dissolve-modal input{margin-top:10px}.danger-confirm{background:var(--color-danger);color:#fff}.danger-confirm:disabled{opacity:.45;cursor:not-allowed}
