...
import { AlertTriangle, ArrowLeft, Bell, Check, Crown, FileText, ImagePlus, Settings, ShieldCheck, Trash2, Users, X } from 'lucide-vue-next'
...
async function dissolveTeam() {
  if (!team.value) return
  dissolveError.value = ''
  if (dissolveText.value.trim() !== '解散小队') {
  ...
  try {
    await apiPost<void>(`/teams/${team.value.id}/dissolve`, {
      confirmed: true,
      confirmationText: dissolveText.value.trim(),
    })
    app.showToast('小队已解散，成员和群聊已停止使用')
    await app.refreshUserState()
    await router.push('/teams')
  } catch (err) {
    dissolveError.value = err instanceof Error ? err.message : '解散失败'
  } finally {
      ...
      <div class="manage-grid">
        ...
        <aside>
          ...
          <section v-if="isOwner" class="manage-panel danger-panel">
            <h2>危险操作</h2>
            <p>解散后所有成员会自动退出，群聊和队内活动将停止使用。</p>
            <button class="danger-btn" @click="openDissolve"><Trash2 :size="15" />解散小队</button>
          </section>
        </aside>
      </div>
    </template>

    <div v-if="showDissolve && team" class="reject-modal dissolve-modal" @click.self="closeDissolve">
      <div>
        ...
        <div class="dissolve-summary">
          <b>{{ team.name }}</b>
          <span>{{ team.members }} 位成员 · {{ team.activeNow }} 人在线</span>
        </div>
        <label>输入“解散小队”确认<input v-model.trim="dissolveText" class="input" autocomplete="off" /></label>
        ...
        <footer>
          ...
          <button class="btn danger-confirm" :disabled="dissolveBusy || dissolveText.trim() !== '解散小队'" @click="dissolveTeam">{{ dissolveBusy ? '解散中...' : '确认解散' }}</button>
        </footer>
      </div>
    </div>
  </div>
