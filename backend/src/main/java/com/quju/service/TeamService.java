...
public class TeamService {
    ...
    public List<TeamDto> list(String query, boolean includeStopped) {
        ...
        String sql = "select t.* from teams t join users u on u.id = t.owner_id where (? = true or t.status = '正常') and (lower(t.name) like ? or lower(t.tags) like ? or lower(u.nickname) like ? or lower(u.email) like ?) order by t.active_now desc, t.members_count desc";
        return jdbc.query(sql, mapper(), includeStopped, normalized, normalized, normalized, normalized);
    }
